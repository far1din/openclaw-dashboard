        ...
        return content
            .map((block: any) => {
                ...
                if (block.type === 'toolResult') {
                    const text = block.content?.map((c: any) => c.text).filter(Boolean).join('\n');
  ...
  if (!sessionKey) {
    return (
        <div className="flex-1 flex flex-col items-center justify-center bg-gray-50 text-gray-400 p-8 text-center animate-in fade-in duration-500">
            ...
            <p className="max-w-sm mt-2">Select an active agent session from the sidebar to view history and start chatting.</p>
        </div>
    );
  }
    ...
    <div className="flex-1 flex flex-col h-full bg-white relative">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50">
            ...
            {messages.map((msg, idx) => (
                <div key={msg.id || idx} className={clsx("flex w-full", msg.role === 'user' ? "justify-end" : "justify-start")}>
                    <div className={clsx(
                        ...
                        <div className={clsx(
                            "w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-1 shadow-sm",
                             msg.role === 'user' ? "ml-3 bg-blue-600 text-white" : "mr-3 bg-white border border-gray-200 text-indigo-600"
                        )}>
